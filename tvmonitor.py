...
<body>
    <div class="container-fluid py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <button onclick="clearAnalytics()" class="btn btn-danger btn-clear px-4">
                <i class="bi bi-trash"></i> Clear Analytics
            </button>
        </div>
        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="card-custom p-4">
                    <h5><i class="bi bi-megaphone text-warning me-2"></i>Ad Insertion (SCTE-35) Status</h5>
                    <div class="row" id="ad-status-row"></div>
                </div>
            </div>
        </div>

        <!-- Countries, Channels, Timeline (same as before) -->
        <div class="row mb-4">
            <div class="col-12"><div class="card-custom p-4"><h5>Top Countries</h5><div class="row" id="countries-row"></div></div></div>
        </div>
    ...
    <script>
        ...
        function updateDashboard() {
            fetch('/api/analytics').then(r => r.json()).then(data => {
                    ...
                    const html = `
                        <div class="col-md-4 mb-3">
                            <div class="card-custom p-3">
                                ...
                                <span class="badge ${ad.last_ad_minutes_ago < 15 ? 'bg-success' : 'bg-secondary'}">
                                    ${ad.status}
                                </span>
                                <small class="text-muted d-block">
                                    ${ad.ad_breaks_detected} breaks detected<br>
                                    Last ad: ${ad.last_ad_minutes_ago} min ago
                                </small>
                            </div>
                        </div>`;
                ...
                // Rest of dashboard (countries, channels, chart) - update as needed
                // ... (you can keep the full logic from previous version)
            });
        }
...
def clear_analytics_route():
    return jsonify({"success": clear_analytics_data()})
